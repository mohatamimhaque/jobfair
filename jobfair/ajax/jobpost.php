...
else
{
  ...
  $u='';
	$query = "
		SELECT * FROM jobpost WHERE status = '0'
	";
...
if(isset($_POST["category"])){
    $category_filter = implode("','", $_POST["category"]);
      $query .= "
         AND post_category IN('".$category_filter."')
          ";
}
...
<?php
    ...
    ?>
     <div class="jobs-container">
        <a href="<?=base_url('view/jobpost/'.$row['jobpost_id']) ?>" class="post"><?= $row['post_name'] ?></a>
        <?php
    ...
    if(mysqli_num_rows($query) > 0){
        foreach($query as $r){
            ?>
         <a href="<?=base_url('profile/corporate/'.$r['user_id']) ?>" class="company"><?= $r['name'] ?></a>
         <?php }} ?>
        <p class="education"><strong>Quantity:</strong> <?= $row['post_number'] ?></p>
         <p class="skills"><strong>Job Location: </strong> <?= $row['job_location'] ?></p>
    </div>           
